...
<script language="javascript">
	...
	$(function() {
		...
		dataGrid = $("#dataGrid").datagrid({
			url : "${ctx}/document/posterList.do",
			striped : true,
			singleSelect:true,
			rownumbers : true,
            pagination:true,
	...
	function addFun() {
		top.showTopDialog("新增页面", "${ctx}/document/saveOrUpdatePosterPage.do", "800", "420","no");
	}
	...
	function updateFun() {
		...
		if (rows!="") {
			if(rows.length>1){
				...
				top.showTopDialog("编辑页面", "${ctx}/document/saveOrUpdatePosterPage.do?id=" + rows[0].id, "800", "420","no");
			}
		} else {
        ...
        if (rows!="") {
            top.$.messager.confirm("确认对话框", "您是否要删除当前数据？", function(b) {
                if(b){
                    $.ajax({
                        type: "POST",
                        url: "${ctx}/document/deletePoster.do",
                        data: {
                            "id" : rows[0].id
                        },
                        dataType: "json",
