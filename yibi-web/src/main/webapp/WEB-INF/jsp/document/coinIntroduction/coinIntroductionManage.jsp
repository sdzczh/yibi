...
<script language="javascript">
	...
	$(function() {
		...
		dataGrid = $("#dataGrid").datagrid({
			url : "${ctx}/coinIntroduction/getCoinIntroductionList.do",
			striped : true,
			singleSelect:true,
			rownumbers : true,
            pagination:true,
	...
	function addFun() {
		top.showTopDialog("新增页面", "${ctx}/coinIntroduction/doCoinIntroductionPage.do", "1000", "620","yes");
	}
	...
	function updateFun() {
		...
		if (rows!="") {
			if(rows.length>1){
				...
				top.showTopDialog("编辑页面", "${ctx}/coinIntroduction/doCoinIntroductionPage.do?id=" + rows[0].id, "1000", "620","yes");
			}
		} else {
		...
		if (rows!="") {
			...
			top.$.messager.confirm("确认对话框", "您是否要删除当前数据？", function(b) {
				if(b){
					$.ajax({
						type: "POST",
						url: "${ctx}/coinIntroduction/deleteCoinIntroduction.do",
						data: {
							"id" : rows[0].id
						},
						dataType: "json",
				...
				<form id="searchForm">
					...
					<table>
						<tr>
							<th>用户姓名:</th>
							<td>
								<input type="text" id="username" name="username" value="${queryBean.username}" class="dfinput" style="width:160px;float: none;" />
							</td>
							<th>登录名称:</th>
							<td>
								<input type="text" id="loginname" name="loginname" value="${queryBean.loginname}" class="dfinput" style="width:160px;float: none;" />
							</td>
							<td>
								&nbsp;
