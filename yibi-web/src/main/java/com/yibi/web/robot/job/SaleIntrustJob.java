...
@Component
@Log4j
public class SaleIntrustJob extends BaseJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            ...
            if(task == null){
                log.info(task.getCointype()+",【自动卖出挂单】-任务不存在："+taskId);
                return ;
            }
            //判断需要提交订单数量
            int count = RandomUtils.nextInt(task.getCountmin(), task.getCountmax());
            log.info(task.getCointype()+",【自动卖出挂单】-开始执行，执行笔数："+count);
            if(count >0){
                for(int i=0 ;i<count;i++){
                    BigDecimal newPrice = getNewPrice(task);//可直接使用task.bastPrice. 此版本不先调用外部交易价格
                    ...
                    if(newPrice!=null && newPrice.compareTo(BigDecimal.ZERO)==1){
                        ...
                        int resultCode = sendSaleLimitOrder(user, task.getCointype(), CoinType.KN, price.doubleValue(), amount);
                        if(resultCode !=10000){
                            //交易发送不成功
                            if(resultCode == 30015){
                                //余额不足，自动进行充值
                                ...
                                log.info(task.getCointype()+",【自动卖出挂单】-自动充值");
                                i=i-1;
                            }
                        }
                    }
     ...
     * @return
     */
    public int  sendSaleLimitOrder(User user ,Integer orderCoinType,Integer unitCoinType,double price,double amount){
        ...
        String url = systemUrl+"/order/limitPriceSale.action";
        ...
        log.info(orderCoinType+",【自动挂单卖出】接口返回："+res);
