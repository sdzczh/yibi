...
@Component
@Log4j
public class BuyMarketJob extends BaseJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            ...
            if(task == null){
                log.info(task.getCointype()+",【自动买入成交】-任务不存在："+taskId);
                return ;
            }
            //判断需要提交订单数量
            int count = RandomUtils.nextInt(task.getCountmin(), task.getCountmax());
            log.info("【自动买入成交】-开始执行，执行笔数："+count);

            if(count >0){
                for(int i=0 ;i<count;i++){
                    Double totalAmt = nextDouble(task.getNumbermin().doubleValue(), task.getNumbermax().doubleValue(),commonI.orderAmtPriceScale(task.getCointype(),CoinType.KN));
                    User user = userService.selectByPrimaryKey(task.getExcuteuserid());

                    int resultCode = sendBuyMarketOrder(user, task.getCointype(), CoinType.KN, totalAmt);
                    if(resultCode !=10000){
                        //交易发送不成功
                        if(resultCode == 30015){
                            //余额不足，自动进行充值
                            ...
                            log.info(task.getCointype()+",【自动买入成交】-自动充值");
                            i=i-1;
                        }
                    }
                }
    ...
    public int  sendBuyMarketOrder(User user , Integer orderCoinType, Integer unitCoinType, double totalAmt){
        ...
        String url = systemUrl+"/order/marketPriceBuy.action";

        String timpStamp = System.currentTimeMillis()+"";
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("total", totalAmt);
        ...
        log.info(orderCoinType+",【成交买入】接口返回："+res);
